...
class InviteWindow(ctk.CTkToplevel):
    """Popup for dispatching a chatroom invite, with date/time/timezone pickers."""
...
class CommsWindow(ctk.CTkToplevel):
    """Chat window backed by the local IRC relay."""
    def __init__(self, master):
        super().__init__(master)
        ...
        self.send_btn = ctk.CTkButton(
            bottom_frame, text="SEND", font=("Inter", 12, "bold"), fg_color=ACCENT,
    ...
    def toggle_connection(self):
        if not self.comms.running:
            ...
            self.update_chat_ui("[SYSTEM]: Disconnected.")
...
class FileCard(ctk.CTkFrame):
    def __init__(self, master, vault_id, original_name, extract_cb, delete_cb):
        ...
        self.btn_delete = ctk.CTkButton(
            self, text="DELETE", width=65, height=28, font=("Inter", 11, "bold"),
...
class ComradeApp(ctk.CTk):
    ...
    def refresh_vault(self):
        """Clears the container and rebuilds it from the current vault state."""
